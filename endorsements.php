<?php include('includes/header.php');?>
<div id="content" class="endorsements">
	<div class="shell">
		<div id="endorsements-banner" class="banner">
			
		</div>
		<section class="copy">
			

				<ul class="special">
				
					<li>Collin County Sheriff<br /> Terry Box	</li>
					<li>Mayor of McKinney<br />Brian Loughmiller</li>
					<li>Mayor of Allen<br /> Steve Terrell</li>
					<li>McKinney Mayor Pro Tem<br/> Travis Ussery</li>

					<li>Constable Precinct 3 Sammy Knapp</li>
					<li>Collin County Commissioner<br />Duncan Webb	</li>
					<li> Collin County Commissioner<br />Cheryl Williams</li>
					<li>Patriot Texas Conservative Activists</li>
					<li></li>
					<li>Collin County Deputies Association</li>
					<li> McKinney Police Association</li>
					<li>Plano Police Association</li>
					<li>Farmersville Police Employees Association </li>
					<li></li>
			

				</ul>
				<ul>
					<li>L. Cheryl Adams</li>
					<li>Paul and Katy Agruso</li>
					<li>Christy Albano</li>
					<li>Todd Albin</li>
					<li>Nancy Amick</li>
					<li>Allen Ayers</li>
					<li>Calli Bailey</li>
					<li>Tom and Kelly Baird</li>
					<li>Kay Baird</li>
					<li>Doug and Karin Ball</li>
					<li>Kyle Basinger</li>
					<li>Carrie Beaird</li>
					<li>Martha Lee Bean</li>
					<li>Alma Benavides</li>
					<li>Hunter Biederman</li>
					<li>Marigay Black</li>
					<li>Sharon Borgne</li>
					<li>Charity Borserine</li>
					<li>Kristin R. Brown</li>
					<li>Kristin Brown</li>
					<li>Jim Burnham</li>
					<li>Jason Burress</li>
					<li>Lauren Cain</li>
					<li>Warren Caldwell</li>
					<li>Beth Chaney</li>
					<li>Mickey Christakos</li>
					<li>Deandra Grant Clendenin</li>
					<li>Rebecca Collins</li>
					<li>Richard Corbitt</li>
					<li>Sharon Corsentino</li>
					<li>Trish Coy</li>
					<li>Darlina Crowder</li>
					<li>Kelly Gallagher Crowson</li>
					<li>Doritha Cunningham</li>
					<li>Bob and Sharon Curtis</li>
					<li>Tom and Anna Daley</li>
					<li>Pamela DeVault</li>
					<li>David Dobecka</li>
					<li>Alice Dobecka</li>
					<li>Alice Todd Dobecka</li>
					<li>Jeff Domen</li>
					<li>Bob and Vanessa Dry</li>
					<li>Lisa Duffee</li>
					<li>Holly Duke</li>
					<li>William C. Dvorak</li>
					<li>Stephen C. Dyer</li>
					<li>Melinda Eitzen</li>
					<li>Paul Elkins</li>
					<li>Kathy Jane Erickson</li>
					<li>Christopher Farish</li>
					<li>Lee Finley</li>
					<li>Abby Foster</li>
					<li>Richard Franklin</li>
					<li>Chris Fredericks</li>
					<li>Bob Garrey</li>
					<li>Brandon Gibbs</li>
					<li>Greg and Leticia Gibbs</li>
					<li>Jenny Gomez</li>
					<li>Noah Gonzalez</li>
					<li>Julie Wilson Hansen</li>
					<li>Julie Hansen</li>
					<li>John Hardin</li>
					<li>Ron and Lynn Harris</li>
					<li>Margaret and Jason Harsch</li>
					<li>David and Donna Hayes</li>
					<li>Dawn Hedlund</li>
					<li>Dawn R. Hedlund</li>
					<li>Rebecca Green Henigsmith</li>
					<li>Julia Henry</li>
					<li>Lisa Hernandez</li>
					<li>Paul Hewett</li>
					<li>David Housel</li>
					<li>Stephanie Hudson</li>
					<li>Stephanie Hudson</li>
					<li>Dave Hughey</li>
					<li>Justin Johnson</li>
					<li>Jody Johnson</li>
					<li>Diann Jones</li>
					<li>Shawn Keefe</li>
					<li>Pat Kelly</li>
					<li>Justin Kendall</li>
					<li>Mary Ann Brooks Kildebeck</li>
					<li>Gary Knapp</li>
					<li>Mark Kosmoski</li>
					<li>Larry S. Lane</li>
					<li>Martin Lenoir</li>
					<li>Martin and Gail Leyko</li>
					<li>Lori Loftin</li>
					<li>Monika Logan</li>
					<li>Julie Lucio</li>
					<li>Deborah Mackoy</li>
					<li>Tony Mallers</li>
					<li>Kevin Malone</li>
					<li>Harry Manning</li>
					<li>Scott Marquardt</li>
					<li>Lisa Marquis</li>
					<li>Robert Matlock</li>
					<li>Robbie McClung</li>
					<li>Kelly and Wade McClure</li>
					<li>Brad and Julie McKeefer</li>
					<li>Mark and Wendy McMillon</li>
					<li>Billy Meeks</li>
					<li>Michella K. Melton</li>
					<li>Stephen H. Miller</li>
					<li>Malcolm Miranda</li>
					<li>Blake Mitchell</li>
					<li>Debby Moen</li>
					<li>Charles and Andrea Mooney</li>
					<li>Kenneth Moore</li>
					<li>Jim Mueller</li>
					<li>Paul Myers</li>
					<li>Rodney Neal</li>
					<li>Justin Nichols</li>
					<li>Mitch Nolte</li>
					<li>Kay O'Brien</li>
					<li>Tom and Saunda O'Connell</li>
					<li>Dana and Summer Palmer</li>
					<li>Scott Palmer</li>
					<li>George Parker</li>
					<li>John Pfister, Jr.</li>
					<li>Tom and Kara Pfister</li>
					<li>John J. Pfister, Sr.</li>
					<li>Lawrence Praeger</li>
					<li>Charles Quaid</li>
					<li>Julie Quaid</li>
					<li>Christine Qualls</li>
					<li>Megan B. Rachel</li>
					<li>Sharon Ramage</li>
					<li>Gwen Rawls</li>
					<li>Marilil Reeves</li>
					<li>Jennifer Richardson</li>
					<li>Christine Riley</li>
					<li>Linda Risinger</li>
					<li>Laura Roach</li>
					<li>Aaron Robb</li>
					<li>Mona Roise</li>
					<li>Jeremy Rosenthal</li>
					<li>Chris Routt</li>
					<li>Christy Bradshaw Schmidt</li>
					<li>Courtney Schmitz</li>
					<li>John Schomburger</li>
					<li>Mark Scroggins</li>
					<li>Brian and Becky Seals</li>
					<li>Kyle Shaw</li>
					<li>Jeff Shore</li>
					<li>Shep Stahel</li>
					<li>Loretta Keller Stark</li>
					<li>April Steele</li>
					<li>Paul and Megan Stuckle</li>
					<li>Alan Taggart</li>
					<li>John Tatum</li>
					<li>Kyle Therrian</li>
					<li>Rae Ann Copeland Thornhill</li>
					<li>Jerry Tidwell, Jr.</li>
					<li>Jim and Emily Tierney</li>
					<li>Randy and Kim Tinsley</li>
					<li>Taylor Tinsley</li>
					<li>Maria Tu</li>
					<li>Bradley Voyles</li>
					<li>Derk Wadas</li>
					<li>James Whalen</li>
					<li>Carol Wilson</li>
					<li>Elisse Woelfel</li>
					<li>Kay Woods</li>
					<li>Robert J. Woods</li>
					<li>Ronald P. Wright</li>
					<li>Jerry and Jane Yancey</li>
				</uL>
				
		</section>

<?php include('includes/footer.php');?>